<?php
	...
	function SQL_InsertUpdate($Entity="", $EntityAlias="", $Data=array(), $Where="", $Debug=false){
		...
		if($UpdateMode){//UPDATE SQL
		    $SQL="UPDATE tbl$EntityLower SET $SQL_SET UserUUIDUpdated = '".$_SESSION["UserUUID"]."', UserUUIDLocked = 0, DateUpdated = '".date('Y-m-d H:i:s')."', DateLocked = '".date('Y-m-d H:i:s')."' WHERE 1 = 1";
		    if($Where!="")$SQL.=" AND $Where";
		}else{//INSERT SQL
		    $Entity_Value_UUID=GUID();
		    if(isset($Data["{$Entity}UUID"]))$Entity_Value_UUID=$Data["{$Entity}UUID"];
		    $SQL="INSERT INTO tbl$EntityLower ({$Entity}UUID, $SQL_FieldName UserUUIDInserted, UserUUIDUpdated, UserUUIDLocked, DateInserted, DateUpdated, DateLocked) VALUES ('$Entity_Value_UUID', $SQL_FieldValue '".$_SESSION["UserUUID"]."', '".$_SESSION["UserUUID"]."', 0, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."')";
		}
	...
	function MySQLQuery($SQL, $Link="", $Debug=false){
		global $Application, $Echo, $nb_queries, $timer_queries;
		...
		/*$DebugResponse="
		    <span class=\"DebugBackground\">
		        <span class=\"DebugReservedWord\">function</span> <span class=\"DebugIdentifier\">MySQLQuery</span> <span class=\"DebugSymbol\">(</span>
		            ...
		            <span class=\"DebugVariable\">print_r(\$Link)</span> <span class=\"DebugSymbol\">=</span> <span class=\"DebugReservedWord\">print_r($Link)</span>
				<span class=\"DebugSymbol\">)</span>
            </span>
			<hr>
		";*/
		
		if(empty($Link)){$Link=$Application["DatabaseLink"];}
	...
	function MySQLRows($SQL, $SingleRow=false, $Link="", $Debug=false){
	    ...
	    $Rows=array();
	    if($SingleRow){
			$Rows=$Query->fetch_array(MYSQLI_BOTH);
		}else{
			while($ThisRow=$Query->fetch_array(MYSQLI_BOTH)){$Rows[]=$ThisRow;}
		}
		$Query->free();
	    return $Rows;
	}
